defmodule Frankencade.GameAgent do
  ...
  def get_player(playerId) do 
    Agent.get(:rawkets, fn(s) -> HashDict.get(s.players, playerId, %Frankencade.Player{}) end)
  end
  ...
  defp bullet_tick(bullet) do 
    ...
    cond do 
      updated_bullet.age > 20 -> 
        ...
        #Remove Bullet
        Frankencade.Endpoint.broadcast! "rawkets:game", "13", %{i: bullet.id}
      true -> 
        #kill player?
        ...
        #Update Bullet
        Frankencade.Endpoint.broadcast! "rawkets:game", "12", %{i: updated_bullet.id, x: updated_bullet.x, y: updated_bullet.y} 
    end
  end
      ...
      case {head.id == bullet.playerId, kill_radius < 10} do 
        {false, true} ->
            updated_player = %{head | alive: false}
                ...
                Frankencade.Endpoint.broadcast! "rawkets:game", "14", %{i: head.id, ik: bullet.playerId, k: updated_killer.killCount } #kill player
                Frankencade.Endpoint.broadcast! "rawkets:game", "13", %{i: bullet.id} #remove bullet
        _ -> 
