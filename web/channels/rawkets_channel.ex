defmodule Frankencade.RawketsChannel do
    use Phoenix.Channel
    ...
    def handle_in(@message_type_update_player, %{"x" => x, "y" => y, "a" => a, "f" => f, "i" => i}, socket) do 
        old = Frankencade.GameAgent.get_player(i)

        if old do        
          new = %{old | x: x, y: y, angle: a, showFlame: f}
          Frankencade.GameAgent.update_player(new)
        end
    ...
    def handle_in(@message_type_new_player, %{"x" => x, "y" => y, "a" => a, "f" => f, "i" => i}, socket) do
      ...
      Frankencade.GameAgent.add_player(p)
    ...
    def handle_in(@message_type_add_bullet, %{"x" => x, "y" => y, "vX" => vX, "vY" => vY}, socket) do
        b = %Frankencade.Bullet{id: get_current_time <> socket.id, playerId: socket.id, x: x, y: y, vX: vX, vY: vY, age: 0, alive: true}
        Frankencade.GameAgent.add_bullet(b)
    ...
    def handle_in(@message_type_revive_player, %{"i" => i}, socket) do 
        Frankencade.GameAgent.revive_player(i)
        {:noreply, socket}
    end

    def terminate(err, socket) do
      Frankencade.GameAgent.remove_player(socket.id)
    ...
    defp decode(str) do
        ...
        tuples = Enum.map(chunks, fn([x,y]) -> {x, Poison.decode!(y, as: Frankencade.Player)} end) 
        Enum.into(tuples, %{})
    end
